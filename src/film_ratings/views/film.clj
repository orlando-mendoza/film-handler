     ...
     (form-to [:post "/add-film"]
              ...
              [:div.form-group.col-12
               ...
               (text-field {:class "mb-3 form-control"
                            :placeholder "Enter film name "} :name)]
              [:div.form-group.col-12
               (label :description "Description:")
               (text-area {:class "mb-3 form-controlf"
                           :placeholder "Enter film description"} :description)]
              [:div.form-group.btn-group.col-12
               (map (labeled-radio "rating") (repeat 5 false) (range 1 6))]
              [:div.form-group.col-12.text-center
               (submit-button {:class "btn btn-primary text-center"}
                              "Add")])]]))
...
(defn film-view
  ...
  (page
   [:div.container.jumbotron.bg-lith
    [:div.row
     [:h2 "Film"]]
